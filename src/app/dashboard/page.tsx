...
export default function DashboardPage() {
  ...
  const handleUpdateSettings = async (settings: any) => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stores/settings`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleActivateStore = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stores/activate`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const fetchStoreMetrics = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stores/metrics`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
...
function CreateStore({ setStoreData }: { setStoreData: (data: StoreData) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const checkResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stores/my-store`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      })
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/stores/setup`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  return (
    ...
    >
      <div className="bg-dark-secondary rounded-lg p-8 border border-white/10">
        <h1 className="text-2xl font-bold text-white mb-6">Create Your Store</h1>
